...
import {
	DATA_ELEMENT_LINKAGE,
	TRACKED_ENTITY_ATTRIBUTE_LINKAGE,
} from "../utils/config";
	...
	trackedEntityTypes: [
		{
			...
			shortName: `[SAT] ${EntityTypes.CATEGORIZATION}`.substring(0, 50),
			publicAccess: "rwrw----",
		},
		{
			id: EntityTypesIds.ACTION,
			name: `[SAT] ${EntityTypes.ACTION}`,
			shortName: `[SAT] ${EntityTypes.ACTION}`.substring(0, 50),
			publicAccess: "rwrw----",
		},
	],
};
...
function generateCategories(
	count: number,
	{ activityLinkageId }: { activityLinkageId: string },
): CategoryConfig[] {
...
export function generateBasicTemplate(levels: number) {
	return ({ name, code }: { name: string; code: string }): Config => {
		const categoryId = uid();
		const actionId = uid();
		const categoryToActivityId = uid();
		const statusFieldId = uid();
		const statusOptionSetId = uid();
		const categories = generateCategories(levels, {
			activityLinkageId: actionId,
		});
		return {
			id: slug(name),
			code,
