...
export const metadataMutation: any = {
	...
	params: ({ mode, importStrategy, atomicMode }: any) => ({
...
export function useUpdateMetadata() {
	...
	const updateMetadataFromConfig = async (config: Config) => {
		const metadata = {
			...initialMetadata,
			...generateMetadataFromConfig(config, { meta: initialMetadata }),
		};
		return uploadMetadata(metadata);
	};

	const createMetadataFromConfig = async (
		config: Config,
