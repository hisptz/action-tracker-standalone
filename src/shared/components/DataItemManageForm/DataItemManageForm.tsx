...
export function DataItemManageForm({
	...
	const valueTypes = useMemo(() => {
		if (isEmpty(excludeFieldTypes)) {
			...
			const filteredValueTypes = SUPPORTED_VALUE_TYPES.filter(
				(type) => !excludeFieldTypes?.includes(type),
			);
	...
	return (
		<Modal position="middle" onClose={onCloseClick} hide={hide}>
				...
				<FormProvider {...form}>
					<form className="column gap-16">
						...
						<div className="mandatory-check-container">
							<RHFCheckboxField
								name={`mandatory`}
								label={i18n.t("Field should be mandatory")}
							/>
						</div>
						<div className="show-in-column-field">
							{type === "attribute" && !actionTable ? (
								<RHFCheckboxField
									name={`header`}
									label={i18n.t("Show field as header")}
								/>
							) : (
								<RHFCheckboxField
									name={`showAsColumn`}
									label={i18n.t("Show field as column")}
								/>
							)}
						</div>
					</form>
			...
			<ModalActions>
				<ButtonStrip>
					<Button
						dataTest="cancel-data-item-btn"
						onClick={onCloseClick}
					>
						{i18n.t("Cancel")}
					</Button>
					<Button
						dataTest="add-data-item-btn"
						primary
						onClick={form.handleSubmit(onSubmit)}
					>
						{defaultValue ? i18n.t("Update") : i18n.t("Add")}
					</Button>
				</ButtonStrip>
			</ModalActions>
		</Modal>
