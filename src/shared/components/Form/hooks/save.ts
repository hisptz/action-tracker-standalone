...
export function useFormActions({
	...
	const { instanceMeta, fields, instanceConfig } = useFormMeta({
		id: instanceMetaId,
		type,
	});
	const { show } = useAlert(
	...
	const uploadFiles = async (data: Record<string, any>) => {
		...
		const fileData = fromPairs(
			(await mapSeries(
				fileFields,
				asyncify(async (field: { name: string }) => {
					if (data[field.name]) {
		...
		async (rawData: Record<string, any>) => {
			...
			if (type === "program") {
				...
				if (defaultValue) {
					...
					//Figure out if this is an action, then create a relationship to the main category
					if (
						instanceConfig?.type === "program" &&
						!!instanceConfig?.parent
					) {
						if (parent && tei && config) {
							const relationship = generateRelationship({
								...
								parentConfig: instanceConfig.parent,
							});
				...
				if (defaultValue) {
					...
					if (!parent || !parentConfig) {
						throw new Error(
							"Parent instance is required for events",
						);
					}
					let trackedEntity;
					let enrollment;
