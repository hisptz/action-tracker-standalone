...
export const sharingEntitySchema = z.object({
	id: z.string(),
	access: z.string(),
});
...
export const dataFieldSchema = z.object({
	...
	native: z
		.boolean({ description: "This field should be in every configuration" })
		.optional(),
	hidden: z
		.boolean({
			description:
				"Hidden from normal user operations. Normally linkages",
		})
		.optional(),
	isStartDate: z
		.boolean({
			description:
				"Field should be flagged as start date (Works for action only for now)",
		})
		.optional(),
	isEndDate: z
		.boolean({
			description:
				"Field should be flagged as end date (Works for action only for now)",
		})
		.optional(),
});
...
export const configSchema = z.object({
	...
	name: z.string({
		description:
			"Name of the configuration (useful for multiple configurations)",
	}),
