...
function generateDataItemsFromConfig(
	field: DataField,
	meta: MetaMeta,
): TrackedEntityAttribute | DataElement {
	...
	return {
		...
		domainType: "TRACKER",
		optionSet:
			field.optionSet?.id !== undefined ? field.optionSet : undefined,
	};
}

function generateProgramFromConfig(
	...
	const programTrackedEntityAttributes: ProgramTrackedEntityAttribute[] =
		config.fields
			.map((field) => generateDataItemsFromConfig(field, meta))
			.map((trackedEntityAttribute) => {
				const teiConfig = find(config.fields, [
					"id",
...
function generateProgramStageFromConfig(
	config: CategoryConfig | ActionStatusConfig,
	{
		meta,
		programId,
		index,
	...
	const dataElements = fields.map((field) =>
		generateDataItemsFromConfig(field, meta),
	) as DataElement[];
...
function generateCategoriesMetadata(
	categories: CategoryConfig[],
	meta: MetaMeta,
) {
	const trackedEntityType = find(meta.trackedEntityTypes, [
		"id",
		EntityTypesIds.CATEGORIZATION,
	]);
	if (categories.length === 1) {
		return {
			program: {
				...generateProgramFromConfig(
					categories[0],
	...
	const programStages = restCategories?.map((category, index) =>
		generateProgramStageFromConfig(category, {
...
function generateActionsMetadata(actionConfig: ActionConfig, meta: MetaMeta) {
	const trackedEntityType = find(meta.trackedEntityTypes, [
		"id",
		EntityTypesIds.ACTION,
	]);

	if ("parent" in actionConfig && actionConfig.parent) {
		if (
			!find(actionConfig.fields, {
				id: meta.linkageConfig.trackedEntityAttribute,
			})
		) {
	...
	const program = generateProgramFromConfig(
		actionConfig,
		trackedEntityType as TrackedEntityType,
		{ meta },
	);
	const programStage = generateProgramStageFromConfig(
		actionConfig.statusConfig,
		{
			programId: program.id,
			meta,
			index: 0,
...
function generateRelationshipTypes(config: Config, meta: MetaMeta) {
	...
	const actionRelationType = {
		...
		fromConstraint: {
			relationshipEntity:
				actionConfig.parent?.type === "program"
					? "PROGRAM_INSTANCE"
					: "PROGRAM_STAGE_INSTANCE",
	...
	return programs.map((program) => {
		return {
			...
			programStages: filter(
				stages,
				(stage) => stage.program?.id === program.id,
			)?.map((programStage) => {
				return {
					id: programStage.id,
...
function cleanProgramStagesDeps(programStages: ProgramStage[]) {
	return programStages.map((programStage) => {
		return {
			...programStage,
			programStageDataElements:
				programStage.programStageDataElements?.map((pstd) => {
					return {
						...pstd,
						dataElement: {
...
export function generateMetadataFromConfig(
	...
	const trackedEntityAttributes =
		compact(extractTrackedEntityAttributes(programs)) ?? [];
