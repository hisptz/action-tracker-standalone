...
import {
  downloadExcel,
  getPdfDownloadData,
} from '../../../core/services/downloadFilesService';
...
export default function ChallengeList() {
  ...
  const { filteredTeis, loading: filteredTeisLoading } = useGetFilteredTeis(
    selectedStatus,
    orgUnit
  );
      ...
      <Grid container spacing={5} direction="column">
        <Grid item style={styles.mainHeaderContainer}>
          <MainPageHeader
            ...
            onAddIndicatorClick={(_) =>
              onModalClose((_) => setAddIndicatorOpen(true))
            }
          />
        </Grid>
        {(loading || filteredTeisLoading) && (
          <Grid item style={styles.fullPage}>
            <FullPageLoader />
          </Grid>
        )}
          ...
          <>
            {_.isEmpty(data.indicators?.trackedEntityInstances) ? (
              <Grid item style={styles.fullPage}>
                {' '}
                <EmptyChallengeList
                  onAddIndicatorClick={(_) => setAddIndicatorOpen(true)}
                />
              </Grid>
            ) : (
              <Grid item container spacing={0} direction="column">
                {_.map(
                  data.indicators?.trackedEntityInstances,
                  (trackedEntityInstance) => {
                    const indicator = new Bottleneck(trackedEntityInstance);
                    return (
                      <Grid
                        key={`${indicator.id}-grid`}
                        item
                        style={styles.card}
                      >
                        <ChallengeCard
