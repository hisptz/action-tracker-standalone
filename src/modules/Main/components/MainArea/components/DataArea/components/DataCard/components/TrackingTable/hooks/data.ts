...
export function useTrackingTableData({ instance }: { instance: any }) {
	const { config } = useConfiguration();
	const { period } = useDimensions();
	const { loading, data, refetch } = useDataQuery<Response>(trackingQuery, {
		variables: {
	...
	const events = useMemo(
		() => data?.events?.instances ?? data?.events?.events ?? [],
		[data?.events],
	);
