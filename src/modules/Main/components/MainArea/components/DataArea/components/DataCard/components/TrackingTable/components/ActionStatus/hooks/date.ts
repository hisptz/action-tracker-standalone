...
export function useShowActionTracking({
    ...
    const actionStartJSDate = new Date(
		action.attributes.find(
		    ({ attribute }) => startDateField?.id === attribute,
		)?.value as string,
    );

    const actionEndJSDate = new Date(
		action.attributes.find(
    ...
    if (
        isNaN(actionEndJSDate.valueOf()) ||
		isNaN(actionStartJSDate.valueOf())
    ) {
        return false;
    }

    const endDateField = actionConfigFields?.find((field) => field.isEndDate);
    const actionStartDate = DateTime.fromJSDate(actionStartJSDate);
    const actionEndDate = DateTime.fromJSDate(actionEndJSDate);
