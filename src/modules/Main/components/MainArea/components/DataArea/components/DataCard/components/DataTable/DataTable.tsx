...
export function DataTable({
	...
	const config = useMemo(() => {
		...
		const categoryConfig = allConfig?.categories?.find(
			(category) => category.id === child.to,
		);
		if (categoryConfig) {
	...
	const { loading, noData, refetch, rows, pagination } = useTableData(
		child.type,
		{
			parentInstance,
			parentType,
	...
	const { columns, childTableColSpan } = useTableColumns(config);

	const trackingColumns = useTrackingColumns();

	const [editedInstance, setEditInstance] = useState();

	const { onDelete } = useDeleteInstance(child.type, {
		instanceName: config?.name as string,
	});
	const onComplete = () => {
		refetch();
	};
	...
	const onDeleteClick = (instance: TrackedEntity | Event) => {
		confirm({
			...
			message: i18n.t(
				"Are you sure you want to delete this {{title}} and all related data?",
				{
					title,
				},
			),
					...
					<tr>
						<td colSpan={columns?.length + childTableColSpan}>
							<div
								...
								className="column center align-center w-100 h-100"
							>
				...
				<tbody>
					<tr>
						<td colSpan={columns?.length + childTableColSpan}>
							<div
								style={{ minHeight: "100px" }}
								className="column center align-center w-100 h-100 gap-16"
							>
								{i18n.t(
									"There are no recorded {{ instanceType }}. Click on the button below to create one",
									{
										instanceType,
									},
								)}
							...
							<TableRow key={`${row.id}-${index}`}>
								...
								{(config as any)?.child && (
									<TableCell
										...
										colSpan={`${childTableColSpan}`}
										key={`${row.id}-child`}
									>
										<DataTable
											nested
											key={`${row.id}-child-table`}
											parentConfig={config as any}
											instance={row.instance}
				...
				<div>
					...
					>
						<Button onClick={onShow} icon={<IconAdd24 />}>
							{i18n.t("Add {{instanceType}}", { instanceType })}
						</Button>
					</AccessProvider>
				</div>
