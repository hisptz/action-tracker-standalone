...
export function ActionStatus({
	...
	const tableData = useMemo(() => {
		...
		const data = compact(
			dataValues.map(
				(dataValue: { dataElement: string; value: string }) => {
					...
					if (
						...
						actionStatusConfig?.stateConfig?.dataElement
					) {
						return {
							name: dataElement?.name,
							value: find(options, ["code", dataValue?.value])
								?.name,
						};
					}
	...
	if (
		period.start.diffNow("days").days > 0 &&
		!selectedPeriod?.interval.engulfs(period.interval)
	) {
		return <TableCell className={classes["tracking-value-cell"]} />;
	}
	const onDelete = () => {
	...
	const status = find(statusEvent.dataValues, [
		"dataElement",
		actionStatusConfig?.stateConfig?.dataElement,
	])?.value;
	...
	return (
		<td
			style={{
				background: hexToRgba(color, 0.4) ?? "transparent",
				width: "auto",
			}}
			className={classes["tracking-value-cell"]}
		>
						...
						<Fragment key={dataValue.name}>
							...
							<span className="m-0">
								<DataView
									...
									value={dataValue.value}
									instanceConfig={
										actionStatusConfig as ActionStatusConfig
									}
