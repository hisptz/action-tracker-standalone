...
export function useDownload() {
	...
	const resource = useMemo(
		() =>
			(serverVersion?.minor ?? 40) > 40 ? "trackedEntities" : "instances",
		[serverVersion],
	);
				...
				...fromPairs(
					compact(
						config.fields.map((field) => {
							...
							return [
								field.name,
								find((instance as TrackedEntity).attributes, {
									attribute: field.id,
								})?.value,
							];
						}),
			...
			...fromPairs(
				compact(
					config.fields.map((field) => {
						...
						return [
							field.name,
							find((instance as Event)?.dataValues, {
	...
	function getStatusData(instance: TrackedEntity) {
		return fromPairs(
			trackingPeriods.map((column) => {
				const event = head(instance.enrollments)?.events.find((event) =>
					column?.interval.contains(
						DateTime.fromISO(event.occurredAt as string),
					),
				);
				if (!event) {
					return [column.name, ""];
				...
				const formattedValue =
					config?.action.statusConfig.fields.reduce(
						(value, field) => {
							if (field.hidden) return value;

							let dataValue = event.dataValues.find(
								({ dataElement }) => dataElement === field.id,
							)?.value;

							if (
								field.id ===
								config?.action.statusConfig.stateConfig
									.dataElement
							) {
								dataValue = statusOptionSet?.options.find(
									(option) => option.code === dataValue,
								)?.name;
							}
	...
	const getChildren = async ({
		...
		if (parentConfig.child.type === "programStage") {
			const childEvents = events?.filter(
				({ programStage, dataValues }) =>
					programStage === parentConfig.child.to &&
					find(dataValues, {
						dataElement: config?.meta.linkageConfig.dataElement,
					})?.value ===
						((parent as Event)?.event ??
							(parent as TrackedEntity)?.trackedEntity),
			);
			return mapSeries(childEvents, async (event: Event) => {
				const parentConfig = find(config?.categories, {
	...
	const { download, downloading, getDownloadData } = useDownloadData({
		query,
		queryKey: "data",
		resource:
			(serverVersion?.minor ?? 40) > 40 ? "trackedEntities" : "instances",
