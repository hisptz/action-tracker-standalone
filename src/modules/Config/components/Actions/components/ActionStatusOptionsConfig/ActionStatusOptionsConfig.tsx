...
export function ActionStatusOptionsConfig() {
	...
	const { options, optionSet, refetch, loading, error } = useStatusOptions();
	...
	return (
		<div className="column gap-16">
			...
			<Table>
				...
				<TableBody>
					{options?.map((option) => {
						const { id, name, code, style } = option;
						return (
							<TableRow key={`${id}-row`}>
									...
									<ActionButton
										...
										onDelete={() => {
											confirm({
												...
												message: (
													<>
														{`${i18n.t("Are you sure you want to delete the option ")}`}
														<b>{option.name}</b>?{" "}
														{i18n.t(
															"Deleting this option may cause some issues if it was used in tracking actions.",
														)}
													</>
												),
											});
			...
			<div className="row end">
				<Button
					dataTest="action-status-option-add-btn"
					onClick={onShow}
				>
					{i18n.t("Add option")}
				</Button>
