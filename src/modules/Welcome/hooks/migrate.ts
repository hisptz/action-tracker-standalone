...
export function useMigrateData() {
	...
	const { refetch, loading: gettingBottlenecks } = useDataQuery(query, {
		lazy: true,
	});
	const { refetch: getActions, loading: gettingActions } = useDataQuery(
		relationshipQuery,
		{ lazy: true },
	);
	const { mutateAsync: upload, isPaused: uploading } = useMutation<
		any,
		any,
		any
	>({
		mutationKey: ["data-migrate"],
		mutationFn: async (variables) => engine.mutate(mutation, { variables }),
	});

	const onEachBottleneckMigrate = async (
		...
		const gapEvents = events.filter(
			({ programStage }) => programStage === head(childCategories)?.id,
		);
		const solutionEvents = events.filter(
			({ programStage }) => programStage === last(childCategories)?.id,
		);
		...
		const updatedSolutionEvents = gapEvents.map((event) => {
			const children = solutionEvents.filter((solutionEvent) => {
				...
				const gapValue = find(event.dataValues, {
					dataElement: linkageId,
				})?.value;
				const solutionValue = find(solutionEvent.dataValues, {
					dataElement: linkageId,
				})?.value;
		...
		const actions = actionsResponse.data?.instances?.map(
			({ to }) => to.trackedEntity,
		);

		const updatedActions = solutionEvents.map((solutionEvent) => {
			const solutionActions = actions?.filter((action) => {
				const actionLinkageValue = find(action.attributes, {
					attribute: "Y4CIGFwWYJD",
				})?.value;
				const solutionLinkageValue = find(solutionEvent.dataValues, {
					dataElement: "prX0q7amAni",
				})?.value;
	...
	const onEachPageMigrate = async (
		page: number,
		{ program, config }: { config: Config; program: string },
	) => {
		...
		return await mapSeries(
			data.data.instances,
			asyncify(
				async (instance: TrackedEntity) =>
					await onEachBottleneckMigrate(instance, { config }),
			),
		);
	};
	...
	const migrate = useCallback(
		async (config: Config) => {
			...
			const pages = Array.from(
				Array(
					Math.ceil(pagination.data.total / pagination.data.pageSize),
				).keys(),
			).map((key) => key + 1);
