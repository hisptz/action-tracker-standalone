...
const programsQuery = {
	meta: {
		resource: "programs",
		params: {
			filter: [`id:in:[${BOTTLENECK_PROGRAM_ID},${ACTION_PROGRAM_ID}]`],
			fields: [
				"id",
				"name",
...
export function useSetupMetadata() {
	const { migrate, progress } = useMigrateData();
	const { updateMetadataFromConfig } = useUpdateMetadata();
	const { data, loading, refetch, engine } = useDataQuery<{
	...
	const setupConfiguration = useCallback(async () => {
		if (!data) {
			throw Error(
				"Something went wrong. Could not find existing metadata",
			);
		}
		...
		return await engine.mutate(
			generateConfigCreateMutation(generatedConfig.id),
			{
				variables: {
	...
	const { mutateAsync: setup, isPending: isLoading } = useMutation({
		mutationKey: ["metadata"],
		mutationFn: setupConfiguration,
		retry: false,
	});
